...
from src.data.data_generator import (
    calculate_ITTE_torch,
    potentialOutcomeSimulation_torch,
)
...
def PEHNE(config, data, model, num_networks, train_mean_y, train_std_y):
    ...
    if do_steps:
        ...
        print("T_list", T_list)
    for T in T_list:
        # create a random tensor of size length with values 0 and 1 with the number of ones equal to T
        ...
        if config["model_type"] == "NetEst" or config["model_type"] == "GINNetEst" or config["model_type"] == "TargetedModel_DoubleBSpline":
            # get z for each node:
            neighbors = torch.sum(A, 1)
            z = torch.div(torch.matmul(A, T_tensor.reshape(-1)), neighbors)
        else:
            z = data.z
        if config["model_type"] == "TargetedModel_DoubleBSpline":

            y_pred_T = model.infer_potential_outcome(
                data.x, T_tensor, z, data.edge_index
            ).squeeze()
        else:
        ...
        if config["model_type"] == "TargetedModel_DoubleBSpline":
            ...
            y_pred_0= model.infer_potential_outcome(
                data.x, zero_tensor, zero_tensor, data.edge_index
            ).squeeze()
        else:
