...
from src.methods.Causal_models import GINModel,NetEst,SPNet,GINNetEst,HINet, HINet_no_net_conf,TARNet,GCN_DECONF,SPNet
from src.utils.utils import Normalize_outcome,Normalize_outcome_recover
from src.utils.metrics import PEHNE,CNEE
...
class Trainer:
    ...
    def train_test_best_model(self,epochs_range,lr_range,alpha_range,hidden_range,num_seeds):
        ...
        if not alpha_tuning:
            ...
            #find elbow
            best_alpha = self.select_alpha(best_epoch,best_lr,best_hidden,alpha_range)
        ...
        wandb.log({"avg_test_y_loss":avg_test_y_loss,"avg_pehne":avg_pehne,"avg_test_cf_y_loss":avg_test_cf_y_loss,"avg_ITTE_loss":avg_ITTE_loss,"avg_cnee":avg_cnee,
    ...
    def train_model(self,train_data,val_data,test=False):
        ...
        if self.model_type == "NetEst" or self.model_type=="GINNetEst":
            optimizer_t = torch.optim.Adam(self.model.discriminator.parameters(), lr=learning_rate,weight_decay=self.config["weight_decay"])
            optimizer_z = torch.optim.Adam(self.model.discrimnator_z.parameters(), lr=learning_rate,weight_decay=self.config["weight_decay"])
            optimizer_p = torch.optim.Adam([{'params':self.model.encoder.parameters()},{'params':self.model.predictor.parameters()}], lr=learning_rate,weight_decay=self.config["weight_decay"])
        else:
            optimizer = torch.optim.Adam(self.model.parameters(), lr=learning_rate,weight_decay=self.config["weight_decay"])
        ...
        for epoch in range(epochs):  
            ...
            for batch in loader:
                ...
                if self.model_type =="NetEst" or self.model_type=="GINNetEst":
                    ...
                    batch_t_loss = BCE_loss(t_pred[:batch_size].squeeze(1),batch.t[:batch_size])
                    batch_z_loss = BCE_loss(z_pred[:batch_size].squeeze(1),batch.z)
                    total_loss = batch_y_loss - alpha*batch_t_loss - gamma*batch_z_loss
                    optimizer_p.zero_grad()
                    total_loss.backward()
                    optimizer_p.step()
                    wandb.log({"batch_y_loss":batch_y_loss.cpu().item(),"batch_t_loss":batch_t_loss.cpu().item(),"batch_z_loss":batch_z_loss.cpu().item(),"batch_total_loss":total_loss.cpu().item()})

                elif self.model_type == "HINet" or self.model_type == "HINet_no_net_conf":
                    total_loss = batch_y_loss_train + alpha*batch_t_loss_train
                    wandb.log({"batch_y_loss":batch_y_loss.cpu().item(),"batch_t_loss":batch_t_loss.cpu().item(),"batch_total_loss":total_loss.cpu().item()})
                    total_loss.backward()
                    optimizer.step()
                elif self.model_type == "SPNet":
                    ...
                    total_loss.backward()
                    optimizer.step()
                    wandb.log({"batch_y_loss":batch_y_loss.cpu().item(),"batch_t_loss":batch_t_loss.cpu().item(),"dLoss":dLoss.cpu().item(),"batch_total_loss":total_loss.cpu().item()})
            ...
            if self.config["track_loss"]:
                ...
                with torch.no_grad():
                    ...
                    #print losses
                    print("epoch",epoch,"batch_num",batch_num,"val_y_loss",val_y_loss,"val_t_loss",
                          val_t_loss,"val_cf_y_loss",val_cf_y_loss,"val_cf_t_loss",val_cf_t_loss)

                    wandb.log({"epoch":epoch+1,"val_y_loss":val_y_loss,"train_y_loss":train_y_loss,"val_t_loss":val_t_loss,"train_t_loss":train_t_loss,
                               "val_cf_y_loss":val_cf_y_loss,"val_cf_t_loss":val_cf_t_loss,"batch_num":batch_num,})
        ...
        with torch.no_grad():
            ...
            print("train_cf loss y",cf_y_train_loss,"train_cf loss t",cf_t_train_loss)
            ...
            if test:
                ...
                test_pehne = PEHNE(self.config,val_data,self.model,self.config["num_networks"],mean_y_train,std_y_train)
                train_pehne = PEHNE(self.config,train_data,self.model,self.config["num_networks"],mean_y_train,std_y_train)
                print("test_pehne",test_pehne)
                print("train_pehne",train_pehne)
                test_cnee =CNEE(self.config,val_data,self.model,self.config["num_networks"],mean_y_train,std_y_train)
                train_cnee =CNEE(self.config,train_data,self.model,self.config["num_networks"],mean_y_train,std_y_train)
                print("test_cnee",test_cnee)
                print("train_cnee",train_cnee)
                wandb.log({"test_pehne":test_pehne,"train_pehne":train_pehne,"test_cnee":test_cnee,"train_cnee":train_cnee,})
                ...
                wandb.log({"val_y_loss_final":val_y_loss,"val_t_loss_final":val_t_loss,"val_cf_y_loss_final":val_cf_y_loss,"val_cf_t_loss_final":val_cf_t_loss,
                        "train_y_loss_final":train_y_loss,"train_t_loss_final":train_t_loss,"train_cf_y_loss_final":cf_y_train_loss,"train_cf_t_loss_final":cf_t_train_loss,
