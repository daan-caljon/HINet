...
def potentialOutcomeSimulation(config, X, A, T, epsilon=0):
    ...
    neighbors = np.sum(A, 1)
    neighborAverage = np.divide(
        np.matmul(A, covariate2NeighborOutcomeMechanism.reshape(-1)), neighbors
    )
...
def generate_data(config, gen_type, nx_seed, watts_strogatz=False):
    ...
    if config["dataset"] == "full_sim" and not do_homophily:
        if watts_strogatz:
            G = nx.connected_watts_strogatz_graph(
                config["num_nodes"], 4, 0.1, seed=config["seed"]
            )
        else:
            G = nx.barabasi_albert_graph(
                config["num_nodes"], config["edges_new_node"], seed=config["seed"]
            )
    ...
    if do_homophily:
        ...
        if os.path.exists(path):
            ...
            A, X = create_homophilous_network(
                config["num_nodes"], config["covariate_dim"]
            )
    ...
    if do_node2vec:
        ...
        if os.path.exists(file):
            ...
            X = node2vec.generate_node_embeddings(
                dense_adj_matrix, embedding_dim=config["covariate_dim"]
            )
    ...
    T, meanT = treatmentSimulation(
        config, X=np.array(X), A=np.array(A)
    )  # effect of X to T
...
def simulate_data(config, setting, watts_strogatz=False):
    train = generate_data(
        config, gen_type="train", nx_seed=config["seed"], watts_strogatz=watts_strogatz
    )

    nx_seed = config["seed"] + 1
    val = generate_data(
        config, gen_type="val", nx_seed=nx_seed, watts_strogatz=watts_strogatz
    )
    nx_seed += nx_seed + 1
    test = generate_data(
        config, gen_type="test", nx_seed=nx_seed, watts_strogatz=watts_strogatz
    )
...
def create_homophilous_network(num_nodes, num_features):
    ...
    for i in range(100):
        ...
        threshold_num_matrix = np.random.normal(
